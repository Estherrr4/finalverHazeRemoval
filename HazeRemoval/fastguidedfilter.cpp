...
FastGuidedFilterMono::FastGuidedFilterMono(const cv::Mat& origI, int r, double eps, int s)
...
FastGuidedFilterColor::FastGuidedFilterColor(const cv::Mat& origI, int r, double eps, int s)
    : FastGuidedFilterImpl(r, eps, s)
{
    cv::Mat I;
    if (origI.depth() == CV_32F || origI.depth() == CV_64F)
